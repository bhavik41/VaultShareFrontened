        ...
        <div className="bg-black/3 border border-vs-border rounded-2xl p-8 backdrop-blur-sm shadow-2xl">
          ...
          {step === "done" ? (
            <div className="flex flex-col items-center gap-5 text-center">
              ...
              <h1 className="text-2xl font-bold text-vs-heading">2FA Enabled!</h1>
              <p className="text-vs-heading/50 text-base">
                Your account is now protected with two-factor authentication. You'll be asked for a code on every sign in.
              </p>
              <button
                onClick={() => navigate("/")}
                className="w-full py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-vs-heading font-semibold transition-colors"
              >
                Back to Dashboard
              </button>
            </div>
            ...
            disableDone ? (
              <div className="flex flex-col items-center gap-5 text-center">
                <CheckCircle2 className="h-16 w-16 text-emerald-600" />
                <h1 className="text-2xl font-bold text-vs-heading">2FA Disabled</h1>
                <p className="text-vs-heading/50 text-base">
              ...
              <div className="flex flex-col items-center gap-5 text-center">
                ...
                <h1 className="text-2xl font-bold text-vs-heading">Disable 2FA</h1>
                <p className="text-vs-heading/50 text-base">
                ...
                <form onSubmit={handleDisable} className="w-full space-y-4 text-left">
                  <div>
                    <label className="block text-base text-vs-heading/70 mb-1.5 text-center">
                    ...
                    <input
                      ...
                      placeholder="000000"
                      className="w-full bg-black/5 border border-vs-border rounded-xl px-4 py-3 text-vs-heading text-center text-xl tracking-[0.5em] placeholder:text-vs-heading/30 focus:outline-none focus:ring-2 focus:ring-rose-500 transition"
                    />
                  </div>
                  {error && (
                  ...
                  <button
                    ...
                    className="w-full py-3 rounded-xl bg-rose-600 hover:bg-rose-500 disabled:opacity-50 text-vs-heading font-semibold flex items-center justify-center gap-2 transition-colors"
                  >
                ...
                <button
                  onClick={() => navigate("/")}
                  className="text-vs-heading/40 hover:text-vs-heading/70 text-base flex items-center justify-center gap-1 transition-colors w-full"
                >
            ...
            <>
                ...
                {(["setup", "verify", "done"] as Step[]).map((s, i) => (
                  <div key={s} className="flex items-center gap-2">
                    <div className={`h-7 w-7 rounded-full flex items-center justify-center text-sm font-bold transition-all ${
                      step === s ? "bg-violet-600 text-vs-heading" :
                      (["setup", "verify", "done"].indexOf(step) > i) ? "bg-emerald-500 text-vs-heading" : "bg-black/5 text-vs-heading/40"
                    }`}>
                      {["setup", "verify", "done"].indexOf(step) > i ? "✓" : i + 1}
                    </div>
                    {i < 2 && <div className="w-8 h-px bg-vs-card/20" />}
                  </div>
                ))}
              </div>
              ...
              {step === "setup" && (
                <div className="flex flex-col items-center gap-5 text-center">
                  ...
                  <h1 className="text-2xl font-bold text-vs-heading">Enable 2FA</h1>
                  <p className="text-vs-heading/50 text-base">
              ...
              {step === "verify" && (
                <div className="flex flex-col items-center gap-5">
                  <h1 className="text-xl font-bold text-vs-heading text-center">Scan QR Code</h1>
                  <p className="text-vs-heading/50 text-base text-center">
                    Open your authenticator app and scan the code below.
                  </p>
                  {qrCode && (
                    <div className="p-3 bg-vs-card rounded-2xl shadow-lg">
                  ...
                  <form onSubmit={handleVerify} className="w-full space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="000000"
                        className="w-full bg-black/5 border border-vs-border rounded-xl px-4 py-3 text-vs-heading text-center text-xl tracking-[0.5em] placeholder:text-vs-heading/30 focus:outline-none focus:ring-2 focus:ring-violet-500 transition"
                      />
                    </div>
                    {error && (
