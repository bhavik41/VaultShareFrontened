    ...
    <div className="flex-1 overflow-y-auto bg-[#f8f9ff]">
      <div className="mx-auto max-w-4xl px-6 py-8">
        <div className="mb-6 flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#ffddb8]/60">
            <Clock size={20} className="text-[#5c3800]" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-[#0b1c30] font-display">Version Requests</h1>
            <p className="text-sm text-[#737784]">Pending version-upload requests for files you own</p>
          </div>
        </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-20">
            <Loader2 size={24} className="animate-spin text-[#003c90]" />
          </div>
        ) : requests.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-4 rounded-xl border border-[#c3c6d5] bg-white p-12 text-center shadow-sm">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#6cf8bb]/20">
              <CheckCircle2 size={24} className="text-[#006c49]" />
            </div>
            <div>
              <h3 className="text-sm font-semibold text-[#0b1c30]">No pending requests</h3>
              <p className="text-sm text-[#737784] mt-1 max-w-sm">You're all caught up. New version-upload requests will show up here.</p>
            </div>
          </div>
        ) : (
          <div className="space-y-3">
            {requests.map(req => (
              <div key={req.id} className="flex items-center justify-between gap-4 rounded-xl border border-[#c3c6d5] bg-white p-4 shadow-sm">
                <div className="min-w-0">
                  <Link to={`/files/${req.fileId}`}
                    className="truncate text-sm font-semibold text-[#003c90] hover:underline">
                  ...
                  {req.changeNote && (
                    <p className="mt-1 text-xs italic text-[#737784]">"{req.changeNote}"</p>
                  )}
                </div>
                <div className="flex shrink-0 items-center gap-2">
                  <button disabled={actionLoadingId === req.id} onClick={() => handleApprove(req)}
                    className="flex items-center gap-1.5 rounded-lg border border-[#006c49]/30 bg-[#6cf8bb]/20 px-3 py-2 text-sm font-semibold text-[#006c49] hover:bg-[#6cf8bb]/30 disabled:opacity-50 cursor-pointer transition-colors">
                  ...
                  <button disabled={actionLoadingId === req.id} onClick={() => handleReject(req)}
                    className="flex items-center gap-1.5 rounded-lg border border-[#ba1a1a]/20 bg-[#ffdad6]/40 px-3 py-2 text-sm font-semibold text-[#ba1a1a] hover:bg-[#ffdad6]/60 disabled:opacity-50 cursor-pointer transition-colors">
                    <X size={13} />Reject
                  </button>
                </div>
              </div>
